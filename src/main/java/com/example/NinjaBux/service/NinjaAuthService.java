...
public class NinjaAuthService {
  ...
  @Autowired private NinjaLoginLogRepository loginLogRepository;

  @Autowired private JwtUtil jwtUtil;

  @Transactional
  public Optional<String> authenticateAndGenerateToken(
  ...
  private void logSuccessfulLogin(Ninja ninja, HttpServletRequest request) {
    try {
      NinjaLoginLog log =
          new NinjaLoginLog(ninja, getClientIp(request), request.getHeader("User-Agent"), true);
  ...
  private void logFailedLogin(String username, HttpServletRequest request) {
    try {
      ...
      if (ninjaOpt.isPresent()) {
        NinjaLoginLog log =
            new NinjaLoginLog(
                ninjaOpt.get(), getClientIp(request), request.getHeader("User-Agent"), false);
        loginLogRepository.save(log);
      }
    } catch (Exception e) {
